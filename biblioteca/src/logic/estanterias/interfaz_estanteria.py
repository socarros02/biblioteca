...
def mostrar_estanterias_disponibles(biblioteca, frame,controller):
    ...
    def mostrar_siguiente():
        ...
        while posicion!=5 and est<len(biblioteca):
            ...
            label.pack(pady=5,expand=True,fill="x",padx=15,side="left")
            btn_ver = ctk.CTkButton(
                fila_frame,
                text="Ver más",
                text_color="white",
                command=lambda estanteria=estanteria_actual['codigo']: mostrar_estanteria(estanteria, controller),
                width=25
            )
            btn_ver.pack(pady=5,padx=15,side="left")
            est+=1
            posicion+=1
        if len(biblioteca)>5:
        ...
        if est> len(biblioteca) - 1 and len(biblioteca)>5:
            controller.borrar_widget(controles)
