...
def mostrar_libros(frame,controller):
    ...
    def mostrar_siguiente():
        ...
        while posicion!=5 and libro<len(controller.libros):
            ...
            lbl_libros.grid(row=posicion, column=0,columnspan=1, padx=5, pady=5, sticky="nsew")

            btn_ver = ctk.CTkButton(
                contenedor,
                text="Ver más",
                text_color="white",
                command=lambda libro=libro_actual:mostrar_libro(libro,controller)
            )
            btn_ver.grid(row=posicion, column=1, padx=5, pady=5, sticky="e")
